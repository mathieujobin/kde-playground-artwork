    ...
    flatMode(false)
{
    ...
    _focusHighlight = /*HACK settings.readNumEntry("/focusHighlight", true)*/fhBack;
                                 ...
                                 const uint flags) const
{
    ...
    if (focused && (_focusHighlight >= fhBack))
    {
        baseColor = alphaBlendColors(baseColor, focusColor, 200);
    }
                               ...
                               bool khtmlMode) const
{
    ...
    if (!flatMode) {
        ...
        if (hasFocus && (_focusHighlight >= fhFrame)) {
            renderContour(p, r, g.background(), getColor(g,FocusHighlight),
                          contourFlags);
        } else {
        ...
        renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                    ...
                    getColor(g, FocusHighlight), _contrast, surfaceFlags);
        if (hasFocus && (_focusHighlight >= fhFrame)) {
            p->setPen( getColor(g,FocusHighlight).dark(130) );
            p->drawLine( r.x()+2, r.y()+1, r.right()-2, r.y()+1 );
            p->drawLine( r.x()+1, r.y()+2, r.x()+1, r.bottom()-2 );
            p->setPen( getColor(g,FocusHighlight).light(130) );
        ...
        renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                      g.background(), g.button(), getColor(g,MouseOverHighlight),
                      getColor(g, FocusHighlight), _contrast/2, surfaceFlags);
        ...
        if(!isSingle) {
            ...
            if( (!isFirst&&!reverseLayout) || (!isLast&&reverseLayout) ) {
                ...
                renderSurface(p, QRect(r.left(), bottom?r.top()+3:(triangular?r.top()+3:r.top()+4), 1, (triangular?r.height()-6:r.height()-7) ),
                            g.background(), g.button(), getColor(g,MouseOverHighlight),
                            getColor(g, FocusHighlight), _contrast,
                                      ...
                                      const QStyleOption& opt) const
{
    ...
    const bool hasFocus = (flags & Style_HasFocus);
...
//  ------
    switch( kpe ) {
        case KPE_SliderGroove: {
            ...
            QColor contour = cg.background().dark(enabled?150:130);

            if (hasFocus && (_focusHighlight >= fhFrame))
                contour = getColor(cg, FocusHighlight);
            ...
            if (horizontal) {
                ...
                if (hasFocus && (_focusHighlight >= fhFrame)) {
                    ...
                    p->drawLine( r.x()+1, center-1, r.right()-1, center-1 );
                    p->setPen( getColor(cg,FocusHighlight).light(130) );
                    p->drawLine( r.x()+1, center, r.right()-1, center );
                }
            } else {
                ...
                if (hasFocus && (_focusHighlight >= fhFrame)) {
                    p->setPen( getColor(cg,FocusHighlight).dark(130) );
                    p->drawLine( center-1, r.y()+1, center-1, r.bottom()-1 );
                    p->setPen( getColor(cg,FocusHighlight).light(130) );
                    p->drawLine( center, r.y()+1, center, r.bottom()-1 );
                }
            }
            break;
        }
        ...
        case KPE_SliderHandle: {
            ...
            QColor contour = getColor(cg,DragButtonContour,s),
            ...
            if (horizontal) {
                ...
                renderSurface(p, QRect(xcenter-4, ycenter-5, 9, 13),
                              ...
                              getColor(cg, FocusHighlight), _contrast+3, surfaceFlags);

                if (hasFocus && (_focusHighlight >= fhFrame)) {
                    if (_focusHighlight >= fhBack)
                       surface = alphaBlendColors( surface, getColor(cg, FocusHighlight), 200 );

                    p->setPen( contour.dark(130) );
                    p->drawLine( xcenter-4, ycenter-4, xcenter-4, ycenter+3 );
                    p->drawLine( xcenter-3, ycenter-5, xcenter+3, ycenter-5 );
                    p->setPen( contour.light(130) );
                    p->drawLine( xcenter+4, ycenter-4, xcenter+4, ycenter+3 );
                    p->drawLine( xcenter+3, ycenter+4, xcenter, ycenter+7 );
                    p->setPen( contour );
                    p->drawLine( xcenter-1, ycenter+6, xcenter-3, ycenter+4 );
                }
                ...
                if (hasFocus && (_focusHighlight >= fhFrame)) {
                    ...
                    p->setPen( contour.dark(130) );
                    p->drawLine( xcenter-4, ycenter-4, xcenter+3, ycenter-4 );
                    p->drawLine( xcenter-5, ycenter-3, xcenter-5, ycenter+3 );
                    p->setPen( contour.light(130) );
                    p->drawLine( xcenter-4, ycenter+4, xcenter+3, ycenter+4 );
                    p->drawLine( xcenter+4, ycenter+3, xcenter+7, ycenter );
                    p->setPen( contour );
                    p->drawLine( xcenter+6, ycenter-1, xcenter+4, ycenter-3 );
                }
                                ...
                                const QStyleOption &opt ) const
{
    ...
    // -------
        ...
        case PE_HeaderSection: {
            ...
            if(!kickerMode) {
                ...
                renderSurface(p, QRect(isFirst?r.left()+1:r.left(), r.top()+1, isFirst?r.width()-2:r.width()-1, r.height()-2),
                                ...
                                getColor(cg, FocusHighlight), _contrast, surfaceFlags);
        ...
        case PE_ButtonCommand: {
            ...
            renderButton(p, r, cg, (on||down), mouseOver, true, enabled, hasFocus, khtmlMode );
            break;
        }
        ...
        case PE_ButtonDefault: {
            ...
            QColor contour = cg.background().dark(120);
            if (hasFocus && (_focusHighlight >= fhFrame))
                contour = alphaBlendColors( contour, getColor(cg,FocusHighlight,enabled), 160 );
            if (!enabled)
                contourFlags|=Is_Disabled;
            renderContour(p, r, cg.background(), contour, contourFlags);
            break;
        }
        ...
        case PE_ScrollBarSlider: {
            ...
            if(r.height() >= 4)
                renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                        ...
                        cg.background(), _contrast+3, surfaceFlags);
    ...
    // -----------------
        case PE_ScrollBarSubLine: {
            ...
            renderSurface(p, QRect(r.left()+1, r.top()+1, r.width()-2, r.height()-2),
                    cg.background(), cg.button(), getColor(cg,MouseOverHighlight),
                    getColor(cg, FocusHighlight), _contrast+3,
                    surfaceFlags);
    ...
    // ----------
        case PE_Indicator: {
            ...
            renderSurface(p, QRect(r.x()+1, r.y()+1, r.width()-2, r.height()-2),
                    ...
                    getColor(cg, FocusHighlight), enabled?_contrast+3:(_contrast/2), surfaceFlags);
    ...
    // --------------
        ...
        case PE_PanelLineEdit: {
            ...
            if ((_focusHighlight >= fhInput) && hasFocus && !isReadOnly && isEnabled)
            {
        ...
        case CE_ProgressBarContents: {
            ...
            if( steps == 0 ) { // Busy indicator
                ...
                renderSurface(p, QRect( r.x()+progress+1, r.y()+1, barWidth-2, r.height()-2 ),
                                    bg, fg, cg.highlight(), cg.highlight(),
                                    2*(_contrast/3),
                ...
                renderSurface(&surfacePainter,
                                ...
                                fg.light(105), fg, cg.highlight(), cg.highlight(), 2*(_contrast/3),
                ...
                renderSurface(&surfacePainter,
                                ...
                                fg, fg.light(105), cg.highlight(), cg.highlight(), 2*(_contrast/3),
                                reverseLayout ? Draw_Right|Draw_Left|Draw_Top|Draw_Bottom|
                                        Round_UpperLeft|Round_BottomLeft|Is_Horizontal
                                : Draw_Right|Draw_Left|Draw_Top|Draw_Bottom|
                                        Round_UpperRight|Round_BottomRight|Is_Horizontal);
...
//                     renderTriangularTab(p, r, cg, (flags & Style_MouseOver), selected, true, pos);
        ...
        case CE_PushButtonLabel:
        {
            ...
            // FIXME
            if ((flags & Style_HasFocus) && (_focusHighlight == fhRect))
    ...
    // -----------------------------------------
        case CE_MenuBarItem: {
            ...
            if (active && focused) {
                // Qt sets Style_HasFocus whether or not we really have focus.
                if (down) {
                    // If we are down, then focus is in our menu, so looking
                    // like we have focus is OK. We also want to do this
                    // because we do sort-of have focus, and also because if
                    // we don't, there is a flicker as the menu pops open.
                    drawPrimitive(PE_ButtonTool, p, r, cg, flags|Style_Down, opt);
                } else {
                    // Otherwise, check, and potentially clear the focus flag
                    // is it is "lying".
            ...
            if (active) {
                if (enabled) {
                    renderSurface(p, r, cg.background(), cg.highlight(), cg.highlight(),
                            cg.highlight(), _contrast+3, Draw_Top|Draw_Bottom|Is_Horizontal);
                }
                else {
                    if ( widget->erasePixmap() && !widget->erasePixmap()->isNull() )
                        p->drawPixmap( r.topLeft(), *widget->erasePixmap(), r );
                    else p->fillRect( r, cg.background().light(105) );
                    if(_focusHighlight) // FIXME - when is this ever drawn?
                        p->drawWinFocusRect( r );
                }
            }
                                     ...
                                     const QStyleOption& opt) const
{
    ...
    // --------
        case CC_ComboBox: {
...
#if 0 // Moved below
            if ((_focusHighlight >= fhInput) && hasFocus && enabled && editable)
            {
...
#else
            if (!hasFocus || !enabled || (_focusHighlight < fhInput) || (!editable && (_focusHighlight < fhFrame)))
#endif
            {
            ...
            renderSurface(p, RbuttonSurface,
                           cg.background(), buttonColor, getColor(cg,MouseOverHighlight),
                           getColor(cg, FocusHighlight), enabled?_contrast+3:(_contrast/2),
                           surfaceFlags);
            ...
            else
            {
                ...
                if (!hasFocus || !enabled || (_focusHighlight < fhInput))
                {
                    // Drawn below if we have focus
            ...
            if (hasFocus && enabled) {
                if ((_focusHighlight >= fhFrame) || ((_focusHighlight >= fhInput) && editable))
                {
                    contourFlags |= Draw_Left|Draw_Right|Draw_Top|Draw_Bottom|
                        Round_UpperLeft|Round_UpperRight|Round_BottomLeft|Round_BottomRight;
                    renderContour(p, r, cg.background(), getColor(cg, FocusHighlight, enabled), contourFlags);

                    // thin frame around the input area
                    p->setPen( getColor(cg, FocusHighlight).light(130) );
                    p->drawLine( r.x()+2, r.bottom()-1, r.right()-2, r.bottom()-1 );
                    p->drawLine( r.right()-1, r.top()+2, r.right()-1, r.bottom()-2 );
                    p->drawLine(reverseLayout?RbuttonSurface.right():RcontentSurface.right(), RcontentSurface.top()+1,
                            reverseLayout?RbuttonSurface.right():RcontentSurface.right(), RcontentSurface.bottom()-1 );

                    p->setPen( getColor(cg, FocusHighlight).dark(130) );
                    p->drawLine( r.x()+1, r.y()+2, r.x()+1, r.bottom()-2 );
                    p->drawLine( r.x()+2, r.y()+1, r.right()-2, r.y()+1 );
                    p->drawLine(reverseLayout?RcontentSurface.x():RbuttonSurface.x(), r.y()+2,
                            reverseLayout?RcontentSurface.x():RbuttonSurface.x(), r.bottom()-1);

                    p->setPen( alphaBlendColors(cg.background(), getColor(cg, FocusHighlight), 50) );
                    p->drawLine(reverseLayout?RbuttonSurface.right()+1:RcontentSurface.right()+1, RcontentSurface.top(),
                            reverseLayout?RbuttonSurface.right()+1:RcontentSurface.right()+1, RcontentSurface.bottom() );
                }
                else if ((_focusHighlight == fhRect) && !editable)
                {
                    drawPrimitive(PE_FocusRect, p,
    ...
    // -----------
        case CC_SpinWidget: {
            ...
            if ((_focusHighlight >= fhInput) && hasFocus && enabled)
            {
                uint editFlags = Draw_Right|Draw_Top|Draw_Bottom|Draw_Left|Round_UpperRight|Round_BottomRight|Round_UpperLeft|Round_BottomLeft;
                renderContour(p, r, cg.background(),
            ...
            if ((_focusHighlight >= fhInput) && hasFocus) {
                p->setPen( alphaBlendColors(cg.background(), getColor(cg, FocusHighlight), 50) );
            } else {
                p->setPen( alphaBlendColors(cg.background(), getColor(cg, ButtonContour, enabled), 50) );
            }
            ...
            // over-paint buttons if we have focus
            if ((_focusHighlight >= fhInput) && hasFocus && enabled) {
                // thin frame around the input area
                p->setPen( getColor(cg, FocusHighlight).dark(130) );
                p->drawLine(reverseLayout?upRect.x()+1:upRect.x(), upRect.y(),
                        reverseLayout?upRect.right():upRect.right()-1, upRect.y());
                p->drawLine(upRect.x(), upRect.y()+1,
                        upRect.x(), upRect.bottom());
                p->drawLine(downRect.x(), downRect.y(),
                        downRect.right(), downRect.y());
                p->drawLine(downRect.x(), downRect.y()+1,
                        downRect.x(), reverseLayout?downRect.bottom()-1:downRect.bottom());

                p->setPen( getColor(cg, FocusHighlight).light(130) );
                p->drawLine(upRect.right(), upRect.y()+1,
                        upRect.right(), upRect.bottom());
                p->drawLine(upRect.x()+1, upRect.bottom(),
                        upRect.right()-1, upRect.bottom());
                p->drawLine(downRect.right(), downRect.y()+1, downRect.right(),
                        reverseLayout?downRect.bottom():downRect.bottom()-1);
                p->drawLine(downRect.x()+1, downRect.bottom(),
                        downRect.right()-1, downRect.bottom());
            }
