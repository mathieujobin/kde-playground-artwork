...
void TileSet::render(const QRect &r, QPainter &p) const
{
    ...
    p.drawPixmap(x0, y0, _pixmap[0]);
    p.drawPixmap(x2, y0, _pixmap[2]);
    p.drawPixmap(x0, y2, _pixmap[6]);
    p.drawPixmap(x2, y2, _pixmap[8]);

    p.drawTiledPixmap(x1, y0, w, _h1, _pixmap[1]);
    p.drawTiledPixmap(x1, y2, w, _h3, _pixmap[7]);
    p.drawTiledPixmap(x0, y1, _w1, h, _pixmap[3]);
    p.drawTiledPixmap(x2, y1, _w3, h, _pixmap[5]);

    p.drawTiledPixmap(x1, y1, w, h, _pixmap[4]);
}
